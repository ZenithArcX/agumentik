...
<body style="background-color: #6ea2f7ff;">
            ...
            <div class="col-md-12">
                ...
                <?php 
                    if(@$_GET['q']==1) 
                    {
                        ...
                        <tr><td><center><b>S.N.</b></center></td><td><center><b>Name</b></center></td><td><center><b>College</b></center></td><td><center><b>Email</b></center></td><td><center><b>IP Address</b></center></td><td><center><b>Retry Control</b></center></td><td><center><b>Action</b></center></td></tr>';
                        $c=1;
                        while($row = mysqli_fetch_array($result)) 
                        {
                            ...
                            $ipResult = mysqli_query($con, "SELECT ip_address FROM user_ip WHERE email='$safeEmail' LIMIT 1") or die('Error fetching user IP');
                            if(mysqli_num_rows($ipResult) > 0)
                            {
                                $ipRow = mysqli_fetch_array($ipResult);
                                $ipAddress = $ipRow['ip_address'];
                            }

                            $isLocked = false;
                            $lockResult = mysqli_query($con, "SELECT id FROM ip_lock WHERE email='$safeEmail' LIMIT 1") or die('Error fetching IP lock');
                            if(mysqli_num_rows($lockResult) > 0)
                            {
                                $isLocked = true;
                            }

                            $lockButton = '';
                            if($ipAddress == 'Not Captured')
                            {
                                $lockButton = '<span class="btn btn-default btn-xs" style="cursor:not-allowed;">No IP Yet</span>';
                            }
                            else if($isLocked)
                            {
                                $lockButton = '<a href="update.php?q=unlockip&email='.$email.'" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-ok-circle" aria-hidden="true"></span>&nbsp;Free Retry</a>';
                            }
                            else
                            {
                                $lockButton = '<a href="update.php?q=lockip&email='.$email.'" class="btn btn-warning btn-xs"><span class="glyphicon glyphicon-lock" aria-hidden="true"></span>&nbsp;Lock Retry</a>';
                            }

                            echo '<tr><td><center>'.$c++.'</center></td><td><center>'.$name.'</center></td><td><center>'.$college.'</center></td><td><center>'.$email.'</center></td><td><center>'.$ipAddress.'</center></td><td><center>'.$lockButton.'</center></td><td><center><a title="Delete User" href="update.php?demail='.$email.'"><b><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></b></a></center></td></tr>';
                        }
                        $c=0;
                        echo '</table></div></div>';
                    }
