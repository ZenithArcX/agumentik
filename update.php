<?php
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['demail']) 
    {
      ...
      $r3 = mysqli_query($con,"DELETE FROM ip_lock WHERE email='$demail' ") or die('Error');
      $r4 = mysqli_query($con,"DELETE FROM user_ip WHERE email='$demail' ") or die('Error');
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'lockip')
    {
      $lockEmail = @$_GET['email'];
      $safeLockEmail = mysqli_real_escape_string($con, $lockEmail);
      $safeAdminEmail = mysqli_real_escape_string($con, $email);

      $ipResult = mysqli_query($con, "SELECT ip_address FROM user_ip WHERE email='$safeLockEmail' LIMIT 1") or die('Error');
      if(mysqli_num_rows($ipResult) > 0)
      {
        $ipRow = mysqli_fetch_array($ipResult);
        $lockIp = mysqli_real_escape_string($con, $ipRow['ip_address']);
        mysqli_query($con, "INSERT IGNORE INTO ip_lock (email, ip_address, locked_by) VALUES ('$safeLockEmail', '$lockIp', '$safeAdminEmail')") or die('Error locking IP');
      }
      header("location:dashboard.php?q=1");
    }
  }
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 'unlockip')
    {
      $unlockEmail = @$_GET['email'];
      $safeUnlockEmail = mysqli_real_escape_string($con, $unlockEmail);
      mysqli_query($con, "DELETE FROM ip_lock WHERE email='$safeUnlockEmail'") or die('Error unlocking IP');
      header("location:dashboard.php?q=1");
    }
  }
  ...
  if(@$_GET['q']== 'quizre' && @$_GET['step']== 25 ) 
  {
    ...
    $currentIp = mysqli_real_escape_string($con, get_client_ip());
    $safeEmail = mysqli_real_escape_string($con, $email);
    $lockCheck = mysqli_query($con, "SELECT id FROM ip_lock WHERE email='$safeEmail' OR ip_address='$currentIp' LIMIT 1") or die('Error checking lock');
    if(mysqli_num_rows($lockCheck) > 0)
    {
