<?php
    ...
    else
    {
        ...
        $safeUserIp = mysqli_real_escape_string($con, $userIp);
        mysqli_query($con, "INSERT INTO user_ip (email, ip_address) VALUES ('$safeEmail', '$safeUserIp') ON DUPLICATE KEY UPDATE ip_address='$safeUserIp', last_seen=NOW()") or die('Error saving user IP');

        $isIpLocked = false;
        $lockQuery = mysqli_query($con, "SELECT id FROM ip_lock WHERE email='$safeEmail' OR ip_address='$safeUserIp' LIMIT 1") or die('Error checking IP lock');
        if(mysqli_num_rows($lockQuery) > 0)
        {
            $isIpLocked = true;
        }
    }
        ...
        <div class="row">
            <div class="col-md-12">
                <?php if(@$_GET['q']==1) 
                {
                    ...
                    if($isIpLocked)
                    {
                        echo '<tr style="color:#99cc32"><td><center>'.$c++.'</center></td><td><center>'.$title.'&nbsp;<span title="This quiz is already solve by you" class="glyphicon glyphicon-ok" aria-hidden="true"></span></center></td><td><center>'.$total.'</center></td><td><center>'.$sahi*$total.'</center></td><td><center><span class="btn sub1" style="color:white;margin:0px;background:#777;cursor:not-allowed"><span class="glyphicon glyphicon-lock" aria-hidden="true"></span>&nbsp;<span class="title1"><b>Retry Locked by Admin</b></span></span></center></td></tr>';
                    }
                    else
                    {
